...
const personaSchema = z.object({ // Creamos un esquema sobre persona, que es un objeto que tiene los atributos del formulario, te faltaría completar eso:
    ...
    dni: z.string().min(7, { message: "El DNI/CUIT debe tener al menos 7 dígitos" }).max(11,{ message:"El DNI/CUIT no debe superar los 11 dígitos"}),
    domicilio: z.string().min(1, { message: "El domicilio es requerido" }),
    localidad: z.string().min(1, { message: "La localidad es requerida" }),
    cp: z.string().min(4, { message: "El codigo postal debe tener al menos 4 dígitos" }).max(10,{ message:"El codigo postal no debe superar los 10 dígitos"}),
    telefono: z.string().min(6, { message: "El telefono debe tener al menos 6 dígitos" }).max(15,{ message:"El telefono no debe superar los 15 dígitos"}),
